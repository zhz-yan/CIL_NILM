...
class BaseLearner(object):
    ...
    def compute_model_size(self):
        """Calculate the total number of parameters in the model (model size)."""
        total_params = sum(p.numel() for p in self._network.parameters())
        return total_params

    def compute_memory_size(self):
        """Calculate the memory size (for exemplars)."""
        feature_size = self.feature_dim  # Feature size (dimensionality of input features)
        memory_size = self.exemplar_size * feature_size  # Total memory size
        return memory_size

    def report_model_and_memory(self):
        """Reports model and memory sizes."""
        model_size = self.compute_model_size()
        memory_size = self.compute_memory_size()

        logging.info(f"Model Size: {model_size} parameters")
        logging.info(f"Memory Size: {memory_size} bytes")

    def update_memory(self, new_data, new_labels):
        """ Update memory with new exemplars (new data and labels)."""
        self._data_memory = np.concatenate([self._data_memory, new_data])
        self._targets_memory = np.concatenate([self._targets_memory, new_labels])
