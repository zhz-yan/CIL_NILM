...
from util import print_model_memory_report
...
class ACIL(BaseLearner):
    ...
    @torch.no_grad()
    def report_per_class(self):
        """在当前 self.test_loader 上计算并打印每类指标。"""
        self._network.eval()
        y_true, y_pred = [], []
        for _, X, y in self.test_loader:
            y = y.to(torch.int64)
            X = X.to(self._device, non_blocking=True)
            # ACILNet.forward 返回 {"logits": ...}
    ...
    def incremental_train(self, data_manager: DataManager) -> None:
        ...
        # self.report_per_class()
        self.report_model_and_memory(tag=f"ACIL task{self._cur_task}")
    ...
    def _init_train(self, train_loader: DataLoader, test_loader: DataLoader) -> None:
        ...
        # 统计“模型 + 优化器状态”的大小
        print_model_memory_report(
            tag="ACIL base (with optimizer)",
            model=self._network,
            optimizer=optimizer,  # 注意：optimizer 此处仍然有效
            mem_features=None, mem_labels=None
        )
    ...
    @torch.no_grad()
    def report_model_and_memory(self, tag: str = ""):
        # ACIL 没有 exemplar replay，故 mem_features/mem_labels 传 None
        # Base 阶段：如果想统计优化器状态，可把 optimizer 传进来；增量阶段 ACIL 是解析式拟合，一般没有优化器状态。
        print_model_memory_report(
            tag=tag or f"ACIL task{self._cur_task}",
            model=self._network,  # ACILNet 是 nn.Module
            optimizer=None,  # base 训练阶段若有 optimizer，可传入以统计
            mem_features=None, mem_labels=None  # exemplar-free -> 0
        )
